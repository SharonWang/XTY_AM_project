...
**Version:** 1.6
**Status:** The supplied 14-notebook 01–09 workflow, Stage 1/2 spatial functions and influence diagnostics, Stage 3 continuous/categorical AM MHCII–AT2 VIM coupling functions, validated Spatial CellChat export, and exploratory continuous AM–AT2 ligand–receptor functions are version-controlled; notebook structure and source references are locally validated, while full-data execution remains an HPC responsibility
**Created:** 2026-09-12
...
direction, tissue, and radius. Multiple core correlations from one donor are
combined by the same equal-core arithmetic mean used in the primary Stage 2
summary; Fisher-z aggregation is retained as a sensitivity analysis. Tested AM
counts are reported but do not weight biological evidence. Removing a core is
explicitly flagged when it also removes a donor who contributed only that core.
Spatial examples may show
supportive cores from distinct donors together with a typical and a
...
1. **Formal Spatial CellChat track in R.** Use tie-safe, equal within-core
   ...
   ambiguous and unassigned AM labels; validate and export a sparse normalized
   or log1p genes-by-cells matrix, cell metadata, and explicitly converted
   micrometre coordinates with a manifest recording expression scale and units; then
...
2. **Exploratory continuous Python track.** For each simple, panel-measured LR
   ...
   Require complete core/donor/tissue provenance and a positive finite
   coordinate conversion before constructing serial, thread, or process tasks.
...
All analytical work will be performed in clearly commented Jupyter notebooks. Python is primary because the source is AnnData/H5AD and the spatial operations use SciPy/Scanpy-compatible structures. A dedicated R notebook may be used where `glmmTMB`, `lme4`, or another validated mixed-model implementation is preferable. The notebooks remain the inspectable scientific record, while reusable Python logic, plot styles, and palettes are maintained in the single documented module `scripts/xty_am_pipeline.py`. Notebook cells must call these functions rather than maintain duplicate implementations.
...
| Balanced score tails are mistaken for discrete AM subtypes | Treat within-core high/low tails as relative sensitivity groups, report the ambiguous middle, retain the continuous score as the primary exposure, and test multiple tail fractions. |
| Averaged neighborhood-enrichment z-scores are overinterpreted | Use donor summaries descriptively; base formal inference on donor-replicated continuous association tests or a prespecified hierarchical model. |
...
| 1.6 | 2026-09-24 | Status; data path; notebook architecture; source API organization; README; validation | Replaced the superseded Notebook 00 with the supplied 14-notebook 01–09 workflow, corrected the local data-root documentation, reorganized all unchanged source definitions into notebook execution order, and added a complete public function-to-notebook index. Added contracts for notebook inventory, JSON structure, pipeline references, exports, source section order, and README coverage; semantic AST comparison confirms that reordering did not alter any function or class body. | Makes the GitHub repository match the current HPC analysis workflow and lets readers trace each notebook to its reusable functions without changing scientific calculations or plotting behavior. |
